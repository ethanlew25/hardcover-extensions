...
var _Sources = (() => {
  ...
  var FF_IMAGE_CDN = "https://zjcdn.mangafox.me";
  var MangaFoxInfo = {
    version: "3.1.0",
    name: "MangaFox",
    icon: "icon.png",
  ...
  var MangaFox = class {
    ...
    async getSearchResults(query, metadata) {
      const page = metadata?.page ?? 1;
      const url2 = new URLBuilder(FF_DOMAIN).addPathComponent("search").addQueryParameter("page", page).addQueryParameter("title", encodeURIComponent(query?.title || "")).addQueryParameter("genres", includedValues(query, "genre").join("%2C")).addQueryParameter("nogenres", excludedValues(query, "genre").join("%2C")).addQueryParameter("type", selectedValue(query, "type", "0")).addQueryParameter("st", selectedValue(query, "completion", "0")).addQueryParameter("rating_method", selectedValue(query, "ratingMethod", "eq")).addQueryParameter("rating", selectedValue(query, "rating", "")).buildUrl();
      const request = App.createRequest({
        url: url2,
        method: "GET"
      });
    ...
    async getSearchTags() {
      ...
      return [
        ...
        ...namespaceTagSections(
          ...
          "genre",
          "exclude",
          /* @__PURE__ */ new Set(["adult", "doujinshi", "hentai", "lolicon", "shotacon", "smut", "yaoi"])
        )
      ];
    }
  };

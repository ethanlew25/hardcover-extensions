...
var _Sources = (() => {
  ...
  // node_modules/.pnpm/cheerio@1.0.0/node_modules/cheerio/dist/browser/static.js
  var static_exports = {};
  ...
  // node_modules/.pnpm/cheerio@1.0.0/node_modules/cheerio/dist/browser/options.js
  var defaultOpts2 = {
  ...
  // node_modules/.pnpm/cheerio@1.0.0/node_modules/cheerio/dist/browser/api/attributes.js
  ...
  // node_modules/.pnpm/cheerio@1.0.0/node_modules/cheerio/dist/browser/utils.js
  ...
  // node_modules/.pnpm/cheerio@1.0.0/node_modules/cheerio/dist/browser/api/traversing.js
  ...
  // node_modules/.pnpm/cheerio@1.0.0/node_modules/cheerio/dist/browser/api/manipulation.js
  ...
  // node_modules/.pnpm/cheerio@1.0.0/node_modules/cheerio/dist/browser/parse.js
  ...
  // node_modules/.pnpm/cheerio@1.0.0/node_modules/cheerio/dist/browser/api/css.js
  var css_exports = {};
  ...
  // node_modules/.pnpm/cheerio@1.0.0/node_modules/cheerio/dist/browser/api/forms.js
  var forms_exports = {};
  ...
  // node_modules/.pnpm/cheerio@1.0.0/node_modules/cheerio/dist/browser/api/extract.js
  var extract_exports = {};
  ...
  // node_modules/.pnpm/cheerio@1.0.0/node_modules/cheerio/dist/browser/cheerio.js
  var Cheerio = class {
    /**
  ...
  // node_modules/.pnpm/cheerio@1.0.0/node_modules/cheerio/dist/browser/load.js
  ...
  // node_modules/.pnpm/cheerio@1.0.0/node_modules/cheerio/dist/browser/parsers/parse5-adapter.js
  ...
  // node_modules/.pnpm/cheerio@1.0.0/node_modules/cheerio/dist/browser/load-parse.js
  ...
  var MANGABALL_DOMAIN = "https://mangaball.net";
  ...
  function parseSearchResults(response) {
    ...
    for (const item of response.data) {
      ...
      if (!mangaId || !item.name?.trim() || item.isAdult === true || looksExplicitTitle(item.name) || containsBlockedTag(tags) || seen.has(mangaId)) continue;
      ...
      results.push(App.createPartialSourceManga({
        ...
        image,
        subtitle: cleanHTML(item.status ?? "") || void 0
      }));
    }
    return results;
  }
  function parseTagSections(response) {
    if (response.code !== 200 || !response.data) return [];
    const preferredGroups = ["genre", "theme", "format"];
    const sections = [];
    for (const group of preferredGroups) {
      const tags = (response.data[group] ?? []).filter((tag) => tag._id && tag.name && !BLOCKED_TAGS.has(tag.name.trim().toLowerCase())).map((tag) => App.createTag({ id: tag._id, label: tag.name.trim() }));
  ...
  function parseMangaDetails(html3, mangaId) {
    const $2 = load(html3);
    const title = $2("#comicDetail h6").first().text().trim();
    const imageNode = $2("img.featured-cover").first();
    const image = secureURL(imageNode.attr("src") ?? "");
    const tags = $2("#comicDetail [data-tag-id]").toArray().map((element) => $2(element).text().trim()).filter(Boolean);
    if (!title || !image) {
    ...
    const alternateTitles = $2(".alternate-name-container").first().text().split("/").map((value) => value.trim()).filter(Boolean);
    const authors = $2("#comicDetail [data-person-id]").toArray().map((element) => $2(element).text().trim()).filter(Boolean);
    const status = $2(".badge-status").first().text().trim();
    const description = $2("#descriptionContent .description-text > p").first().text().trim();
    const tagSections = tags.length > 0 ? [App.createTagSection({
      id: "genres",
      label: "Genres",
  ...
  function parseChapters(response, mangaId) {
    ...
    for (const chapterGroup of response.ALL_CHAPTERS) {
      ...
      for (const translation of chapterGroup.translations ?? []) {
        if (translation.language !== "en" || !translation.id || finiteNumber(translation.pages, 0) <= 0 || seen.has(translation.id)) continue;
        const translationTitle = translation.name?.trim() || chapterGroup.title?.trim();
        const name = translationTitle ? `Chapter ${chapterNumber}: ${translationTitle}` : `Chapter ${chapterNumber}`;
        const rawDate = translation.date?.trim();
        const date = rawDate ? /* @__PURE__ */ new Date(rawDate.replace(" ", "T") + "Z") : void 0;
        ...
        chapters.push(App.createChapter({
          ...
          time: date && !Number.isNaN(date.getTime()) ? date : void 0
        }));
      }
    }
  ...
  function parseChapterDetails(html3, mangaId, chapterId) {
    const encoded = html3.match(/const\s+chapterImages\s*=\s*JSON\.parse\(`([\s\S]*?)`\)/)?.[1];
    if (!encoded) {
    ...
    const pages = Array.isArray(rawPages) ? rawPages.filter((page) => typeof page === "string").map(secureURL).filter(Boolean) : [];
    if (pages.length === 0) {
      throw new Error(`MangaBall returned no pages for chapter ${chapterId}.`);
    }
    return App.createChapterDetails({ id: chapterId, mangaId, pages });
  }
  function parseCSRF(html3) {
    const $2 = load(html3);
    const token = $2('meta[name="csrf-token"]').attr("content")?.trim() ?? "";
    if (!token) throw new Error("MangaBall did not provide a CSRF token.");
    return token;
  }
  function rawTitleID(mangaId) {
  ...
  function fragmentValues(html3, selector) {
    const $2 = load(html3);
    return $2(selector).toArray().map((element) => $2(element).text().trim()).filter(Boolean);
  }
  function cleanHTML(html3) {
    return load(html3).text().replace(/\s+/g, " ").trim();
  }
  function containsBlockedTag(tags) {
    return tags.some((tag) => BLOCKED_TAGS.has(tag.trim().toLowerCase()));
  }
  function looksExplicitTitle(title) {
  ...
  function namespaceTagSections(sections, key, kind = "exclude", blockedLabels = /* @__PURE__ */ new Set()) {
    const result = [];
    for (const section of sections) {
      const tags = (section.tags ?? []).filter((tag) => !blockedLabels.has(tag.label.trim().toLowerCase())).map((tag) => App.createTag({
        id: `${key}:${tag.id}`,
        label: tag.label
      }));
      if (tags.length === 0) continue;
  ...
  // src/MangaBall/MangaBall.ts
  var MANGABALL_DOMAIN2 = "https://mangaball.net";
  var MEDIA_HOST_ROOT = "https://poke-black-and-white.net";
  var SEARCH_PAGE = "/search-advanced/";
  var HOME_SECTIONS = [
    { id: "updated", title: "Latest Updates", sort: "updated_chapters_desc", type: import_types2.HomeSectionType.featured },
    { id: "popular", title: "Most Viewed", sort: "views_desc", type: import_types2.HomeSectionType.singleRowLarge },
    { id: "new", title: "Recently Added", sort: "created_at_desc", type: import_types2.HomeSectionType.singleRowNormal }
  ];
  var MangaBallInfo = {
    version: "1.1.1",
    name: "MangaBall",
    ...
    websiteBaseURL: MANGABALL_DOMAIN2,
    language: "English",
    sourceTags: [],
  ...
  var MangaBall = class {
      ...
      this.requestManager = App.createRequestManager({
        ...
        interceptor: {
          interceptRequest: async (request) => {
            request.headers = {
              ...request.headers ?? {},
              "accept": request.method === "POST" ? "application/json, text/javascript, */*; q=0.01" : "text/html,application/xhtml+xml",
              "referer": `${MANGABALL_DOMAIN2}/`,
    ...
    getMangaShareUrl(mangaId) {
      return `${MANGABALL_DOMAIN2}/title-detail/${encodeURIComponent(mangaId)}/`;
    }
    ...
    async getChapters(mangaId) {
      ...
      return parseChapters(parseJSON(response.data), mangaId);
    }
    ...
    async getSearchTags() {
      ...
      const tags = namespaceTagSections(
        parseTagSections(parseJSON(response.data)),
        "tag",
        "exclude"
      );
      return [
    ...
    pagedResults(response, page) {
      const current = response.pagination?.current_page ?? page;
      const last2 = response.pagination?.last_page ?? current;
      return App.createPagedResults({
        results: parseSearchResults(response),
        metadata: current < last2 ? { page: current + 1 } : void 0
      });
    }
    async search(title, page, sort, includedTags = [], excludedTags = [], includeMode = "and", publicationStatus = "any", originalLanguage = "any", demographic = "any") {
      const token = await this.sessionToken(SEARCH_PAGE);
      const fields = [
        ["search_input", title],
        ["filters[sort]", sort],
      ...
      fields.push(...includedTags.map((tag) => ["filters[tag_included_ids][]", tag]));
      fields.push(...excludedTags.map((tag) => ["filters[tag_excluded_ids][]", tag]));
      const response = await this.post(

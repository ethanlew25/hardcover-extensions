...
var _Sources = (() => {
  ...
  var WEBCENTRAL_DOMAIN = "https://weebcentral.com";
  ...
  function parseSearchPage(html3) {
    const $2 = load(html3);
    const results = [];
    const seen = /* @__PURE__ */ new Set();
    for (const card of $2("article.bg-base-300.flex.gap-4.p-4").toArray()) {
      const root2 = $2(card);
      const link = root2.find('a[href*="/series/"]').first();
      const mangaId = seriesID(link.attr("href") ?? "");
      const title = root2.find('a.line-clamp-1[href*="/series/"]').first().text().trim() || root2.find('img[alt$=" cover"]').first().attr("alt")?.replace(/\s+cover$/i, "").trim() || "";
      if (!mangaId || !title || seen.has(mangaId)) continue;
      const tags = labeledValues(root2, $2, /^Tag\(s\):?$/i);
      if (containsBlockedTag(tags)) continue;
      const status = labeledValue(root2, $2, /^Status:?$/i);
      const type = labeledValue(root2, $2, /^Type:?$/i);
      const year = labeledValue(root2, $2, /^Year:?$/i);
      const subtitle = [type, status, year].filter(Boolean).join(" \u2022 ");
      const image = absoluteURL(root2.find('img[alt$=" cover"]').first().attr("src") ?? "");
      if (!image) continue;
      seen.add(mangaId);
    ...
    return {
      results,
      hasNextPage: $2('button[hx-get*="/search/data"][hx-get*="offset="]').length > 0
    };
  }
  function parseMangaDetails(html3, mangaId) {
    const $2 = load(html3);
    const title = $2("main h1").first().text().trim();
    const image = absoluteURL($2('main img[alt$=" cover"]').first().attr("src") ?? "");
    if (!title || !image) {
      throw new Error(`WeebCentral returned incomplete details for ${mangaId}.`);
    }
    const adult = detailValue($2, /^Adult Content:?$/i);
    const tags = detailValues($2, /^Tags?\(s\):?$/i);
    if (!/^(no|false)$/i.test(adult) || containsBlockedTag(tags)) {
      throw new Error("This title is excluded by the source content filter.");
    }
    const authors = detailValues($2, /^Author\(s\):?$/i);
    const status = detailValue($2, /^Status:?$/i);
    const type = detailValue($2, /^Type:?$/i);
    const released = detailValue($2, /^Released:?$/i);
    const description = $2("main strong").filter((_index, element) => /^Description$/i.test($2(element).text().trim())).first().parent().find("p").first().text().trim();
  ...
  function parseChapters(html3, mangaId) {
    const $2 = load(html3);
    const chapters = [];
    const seen = /* @__PURE__ */ new Set();
    for (const linkElement of $2('a[href*="/chapters/"]').toArray()) {
      const link = $2(linkElement);
      const chapterId = chapterID(link.attr("href") ?? "");
      if (!chapterId || seen.has(chapterId)) continue;
      const name = link.find("span.grow > span").first().text().trim() || link.find("span").first().text().trim() || `Chapter ${chapters.length + 1}`;
      const chapterNumber = chapterNumberFrom(name, chapters.length + 1);
      const rawDate = link.find("time[datetime]").first().attr("datetime");
      const date = rawDate ? new Date(rawDate) : void 0;
      seen.add(chapterId);
  ...
  function parseChapterDetails(html3, mangaId, chapterId) {
    const $2 = load(html3);
    const pages = $2('img[alt^="Page "]').toArray().map((element) => absoluteURL($2(element).attr("src") ?? "")).filter(Boolean);
    if (pages.length === 0) {
      throw new Error(`WeebCentral returned no pages for chapter ${chapterId}.`);
    }
    return App.createChapterDetails({ id: chapterId, mangaId, pages });
  }
  function detailValue($2, pattern) {
    return detailValues($2, pattern)[0] ?? "";
  }
  function detailValues($2, pattern) {
    const label = $2("main strong").filter((_index, element) => pattern.test($2(element).text().trim())).first();
    if (label.length === 0) return [];
    return label.parent().find("a").toArray().map((element) => $2(element).text().trim()).filter(Boolean);
  }
  function labeledValue(root2, $2, pattern) {
    return labeledValues(root2, $2, pattern)[0] ?? "";
  }
  function labeledValues(root2, $2, pattern) {
    const label = root2.find("strong").filter((_index, element) => pattern.test($2(element).text().trim())).first();
    if (label.length === 0) return [];
    return label.parent().find("span").toArray().map((element) => $2(element).text().replace(/,\s*$/, "").trim()).filter(Boolean);
  }
  ...
  var HOME_SECTIONS = [
    { id: "popular", title: "Popular", sort: "Popularity", type: import_types2.HomeSectionType.featured },
    { id: "latest", title: "Latest Updates", sort: "Latest Updates", type: import_types2.HomeSectionType.singleRowLarge },
  ...
  var WeebCentralInfo = {
    ...
    websiteBaseURL: WEBCENTRAL_DOMAIN2,
    language: "English",
    sourceTags: [],
  ...
  var WeebCentral = class {
    ...
    getMangaShareUrl(mangaId) {
      return `${WEBCENTRAL_DOMAIN2}/series/${encodeURIComponent(mangaId)}`;
    }
    ...
    pagedResults(page, offset) {
      return App.createPagedResults({
        results: page.results,
        metadata: page.hasNextPage ? { offset: offset + PAGE_SIZE } : void 0
      });
    }
    async search(title, offset, sort, order = "Descending", official = "Any", anime = "Any", includedStatuses = [], includedTypes = [], includedTags = [], excludedTags = []) {
      const parameters = [
        ["limit", PAGE_SIZE],
        ["offset", offset],
        ["adult", "False"],
      ...
      parameters.push(...includedStatuses.map((status) => ["included_status", status]));
      parameters.push(...includedTypes.map((type) => ["included_type", type]));
      parameters.push(...includedTags.map((tag) => ["included_tag", tag]));
      parameters.push(...excludedTags.map((tag) => ["excluded_tag", tag]));
      const query = parameters.map(([key, value]) => `${encodeURIComponent(key)}=${encodeURIComponent(String(value))}`).join("&");
    ...
    async get(relativeURL) {
      const response = await this.requestManager.schedule(App.createRequest({
        url: `${WEBCENTRAL_DOMAIN2}${relativeURL}`,
        method: "GET"
      }), 1);

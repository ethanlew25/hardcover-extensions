...
var _Sources = (() => {
  ...
  // src/MKissa/MKissa.ts
  var MKissa_exports = {};
  __export(MKissa_exports, {
    MKissa: () => MKissa,
    MKissaInfo: () => MKissaInfo
  });
  var import_types = __toESM(require_lib());

  // src/MKissa/MKissaParser.ts
  var import_html_entities = __toESM(require_lib2());
  var COVER_HOST = "https://aln.youtube-anime.com";
  var BLOCKED_TERMS = /* @__PURE__ */ new Set([
    "adult",
    "doujinshi",
    "ecchi",
    "erotica",
  ...
  function parseMangaDetails(item, mangaId) {
    if (isBlocked(item)) {
    ...
    const labels = unique([...item.genres ?? [], ...item.tags ?? []]).filter((label) => !BLOCKED_TERMS.has(label.trim().toLowerCase()));
    const tagSections = labels.length > 0 ? [App.createTagSection({
      id: "catalog-tags",
      label: "Tags",
  ...
  function isBlocked(item) {
    ...
    return [...item.genres ?? [], ...item.tags ?? []].some((value) => BLOCKED_TERMS.has(value.trim().toLowerCase()));
  }
  ...
  function cleanDescription(value) {
    return (0, import_html_entities.decode)(value.replace(/<br\s*\/?>/gi, "\n").replace(/<[^>]+>/g, " ")).replace(/[ \t]+/g, " ").replace(/\n\s+/g, "\n").trim();
  }
  function unique(values2) {
    const seen = /* @__PURE__ */ new Set();
    return values2.map((value) => value?.trim() ?? "").filter((value) => {
  ...
  // src/MKissa/MKissa.ts
  var MKISSA_DOMAIN = "https://mkissa.to";
  var API_URL = "https://api.mkissa.net/api";
  var COVER_HOST2 = "https://aln.youtube-anime.com";
  var PAGE_SIZE = 24;
  var SAFE_GENRES = [
    "4 Koma",
    "Action",
...
query($ids: [String!]!) {
  ...
  var HOME_SECTIONS = [
    { id: "recent", title: "Recent Catalog Updates", sort: "Latest_Update", type: import_types.HomeSectionType.featured },
    { id: "popular", title: "Popular Manga", sort: "Popular", type: import_types.HomeSectionType.singleRowLarge },
    { id: "alphabetical", title: "Browse A\u2013Z", sort: "Name_ASC", type: import_types.HomeSectionType.singleRowNormal }
  ];
  var MKissaInfo = {
    version: "1.0.1",
    name: "MKissa Catalog",
    ...
    language: "English",
    sourceTags: [],
    intents: import_types.SourceIntents.HOMEPAGE_SECTIONS
  };
  var MKissa = class {
    constructor() {
    ...
    pagedResults(result, page) {
      const results = parseSearchResults(result.items);
      return App.createPagedResults({
        results,
        metadata: page * PAGE_SIZE < result.total ? { page: page + 1 } : void 0
      });
    }
    async graphQL(query, variables) {
      ...
      if (response.status < 200 || response.status >= 300) {
        const detail = typeof response.data === "string" ? response.data.replace(/\s+/g, " ").slice(0, 300) : "";
        throw new Error(
          `MKissa catalog request failed with HTTP ${response.status}` + (detail ? `: ${detail}` : ".")
        );
      }
      return response;
    }
  ...
  return __toCommonJS(MKissa_exports);
})();
